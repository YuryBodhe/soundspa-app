...
async function main(){try{
  ...
  const previousPlaying=make("previous-playing");await previousPlaying.play();await flush();previousPlaying.previous();await flush();assert.equal(previousPlaying.getSnapshot().currentTrackIndex,2);assert.equal(previousPlaying.getSnapshot().status,"playing");previousPlaying.previous();await flush();assert.equal(previousPlaying.getSnapshot().currentTrackIndex,1);
  ...
  const previousPaused=make("previous-paused");previousPaused.previous();await flush();assert.equal(previousPaused.getSnapshot().currentTrackIndex,2);assert.equal(previousPaused.getSnapshot().status,"paused");await previousPaused.play();await flush();assert.equal(previousPaused.getSnapshot().currentTrackIndex,2);
  const oneTrack=make("next-single",[tracks[0]]);oneTrack.next();assert.equal(oneTrack.getSnapshot().currentTrackIndex,0);
  const onePrevious=make("previous-single",[tracks[0]]);onePrevious.previous();assert.equal(onePrevious.getSnapshot().currentTrackIndex,0);
  const fourTracks=[...tracks,{id:"d",url:"/music/d.mp3"}];const rapid=make("next-rapid",fourTracks);await rapid.play();await flush();rapid.next();rapid.next();rapid.previous();await flush();assert.equal(rapid.getSnapshot().currentTrackIndex,1);assert.equal(rapid.getSnapshot().status,"playing");rapid.previous();rapid.previous();rapid.next();await flush();assert.equal(rapid.getSnapshot().currentTrackIndex,1);
